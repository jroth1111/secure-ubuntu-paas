#!/usr/bin/env bats
# Additional behavior tests to close function coverage gaps in base/bootstrap.sh
...
@test "main: verifies Tailscale and emits sentinel before baseline package upgrades" {
  run bash -c '
    source "'"${SCRIPT}"'"
    tmp="$(mktemp)"
    parse_args() { :; }
    ...
    configure_cron_extra_opts() { :; }
    ensure_power_group() { :; }
    install_tailscale() { echo install-tailscale >> "'"${BATS_TEST_TMPDIR}"'/order.log"; }
    require_commands() { :; }
    ensure_tailscaled_notify_access() { echo notify >> "'"${BATS_TEST_TMPDIR}"'/order.log"; }
    verify_tailscale_iface() { echo verify-tailscale >> "'"${BATS_TEST_TMPDIR}"'/order.log"; }
    ensure_tailscale_ssh_disabled() { echo disable-ts-ssh >> "'"${BATS_TEST_TMPDIR}"'/order.log"; }
    apply_system_package_updates() { echo apply-upgrades; echo apply-upgrades >> "'"${BATS_TEST_TMPDIR}"'/order.log"; }
    detect_docker() { :; }
    discover_docker_ssh_cidrs() { :; }
    configure_swap() { :; }
    disable_unused_services() { :; }
    ...
    up_line="$(grep -n "apply-upgrades" "${tmp}" | head -n1 | cut -d: -f1)"
    [[ -n "${sent_line}" && -n "${up_line}" ]]
    (( sent_line < up_line ))
  '
  assert_success
}

@test "main: enforces SSH session safety before host mutations" {
  run bash -c '
    source "'"${SCRIPT}"'"
    tmp="$(mktemp)"
    parse_args() { :; }
    ...
    detect_os() { :; }
    check_disk_space() { :; }
    ssh_session_safety_gate() { echo safety-gate; }
    configure_timezone() { echo configure-timezone; }
    normalize_private_hosts_file() { :; }
    ensure_timesync() { :; }
    detect_wan_iface() { :; }
    ensure_packages() { :; }
    ...
    main > "${tmp}"
    gate_line="$(grep -n "^safety-gate$" "${tmp}" | head -n1 | cut -d: -f1)"
    tz_line="$(grep -n "^configure-timezone$" "${tmp}" | head -n1 | cut -d: -f1)"
    [[ -n "${gate_line}" && -n "${tz_line}" ]]
    (( gate_line < tz_line ))
  '
  assert_success
}
